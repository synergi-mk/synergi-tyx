...
export type LambdaEvent = RemoteEvent &
  LambdaApiEvent &
  LambdaS3Event &
  LambdaDynamoEvent &
  LambdaScheduleEvent;
...
export interface LambdaHandler {
  (event: LambdaEvent, context: LambdaContext, callback: LambdaCallback):
    | boolean
    | void;
}
...
export class LambdaContainer extends ContainerPool {
  ...
  private async handler(event: LambdaEvent, context: LambdaContext) {
    ...
    if (event.httpMethod) {
      ...
      (event.type === "remote" || event.type === "internal") &&
      event.service &&
      event.method
    ) {
      try {
      ...
      event.Records &&
      event.Records[0] &&
      (event.Records[0] as LambdaS3Record).eventSource === "aws:s3"
    ) {
      try {
      ...
      event.Records &&
      event.Records[0] &&
      (event.Records[0] as LambdaDynamoRecord).eventSource === "aws:dynamodb"
    ) {
      try {
  ...
  private async http(
    ...
    let req: HttpRequest = {
      ...
      sourceIp:
        (event.req && event.req.identity && event.req.identity.sourceIp) ||
        "255.255.255.255",
  ...
  private async dynamo(
    event: LambdaDynamoEvent,
    context: LambdaContext
  ): Promise<EventResult> {
